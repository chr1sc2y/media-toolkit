...
def script_description() -> str:
    ...
    lines.extend(
        [
            "",
            "Typical runs:",
            "- Dry-run with fresh Photos scan: python3 scripts/fill_missing_photo_locations.py --force-refresh",
            "- Apply from cached plan data: python3 scripts/fill_missing_photo_locations.py --apply",
        ]
    )
    return "\n".join(lines)
