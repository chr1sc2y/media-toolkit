...
def build_move_plan(root: Path, entries: list[ManifestEntry]) -> list[MoveOperation]:
    return manifest_organize.build_move_plan(root, entries, "panorama")
...
def rebuild_contact_sheets(root: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="panorama-organize-sheets-") as temp:
        temp_dir = Path(temp)
        run_command(
            [
                "mt",
        ...
        if panorama_dir.exists():
            run_command(
                [
                    ...
                    "--hif-only",
                    "--output",
                    str(temp_dir / "panorama"),
                    "--final-overview",
                    str(panorama_dir / "_contact_sheet.jpg"),
...
def summarize(entries: list[ManifestEntry]) -> str:
    return manifest_organize.summarize(entries, "panorama")
...
def organize_panoramas(args: argparse.Namespace) -> int:
    ...
    manifest = (
        ...
        if args.manifest
        else root / "panorama" / "panorama_manifest.tsv"
    )
...
if __name__ == "__main__":
    sys.exit(organize_panoramas(parse_args()))
