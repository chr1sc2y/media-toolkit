...
- 成片归档：用户在 Lightroom 手工精修并导出最终 JPG 后，先运行 `mt preflight-run finalize <照片目录> --copy-to <目标目录> --scene <场景类别>`，通过后运行 `mt finalize <照片目录> --copy-to <目标目录> --photos-album Sony --scene <场景类别>`；如果用户要求只拷贝 HIF，则运行 `mt finalize <照片目录> --copy-to <目标目录> --hif-only --scene <场景类别>`。如果用户没有提供 `--copy-to` 目标目录，必须先询问，不要猜测；只给一个路径时，它只能是源照片目录，不能当作目标目录。不要使用源目录根、源目录下的 `featured/`、`~/Downloads/featured` 或历史记忆里的路径作为默认目标。`--copy-to` 必须在源照片目录外部。该流程以 Lightroom 导出文件为最终名单：根目录 `raw/Export/` 和 `portrait/<n>/raw/Export/` 里的文件名 stem 决定要归档哪些原始 HIF；然后从根目录 `hif/` 和 `portrait/<n>/hif/` 直接复制匹配 HIF 到用户给出的目标目录，不再创建中间 `featured/`。复制 HIF 时保留源文件 metadata，不改 EXIF、文件名、时间戳或图片内容，也绝不删除目标目录已有内容。它不复制 Lightroom 导出文件到 HIF 目标目录，不把 `panorama/<n>/hif/` 里的全景源片 HIF 复制进目标目录，不写本地 style learning 报告。`--photos-album Sony` 会把 `raw/Export/`、`portrait/<n>/raw/Export/`、`panorama/<n>/raw/Export/` 里的 Lightroom 导出图导入 Apple Photos 的 Sony 相册；`--hif-only`/`--no-photos` 跳过 Apple Photos 导入。真实归档成功后，默认继续运行 `mt hif-prune <照片目录> --mode aggressive --scene <场景类别>`，直接删除源目录中高置信冗余 HIF-only 重复图，并写入 `hif_prune_manifest.json` 和同名 TSV；但绝不删除 Export/Pixcake 或 raw/Export 选中 stem 对应的 HIF、仍有同名 RAW 的 HIF、`panorama/<n>/hif/` 全景源片，且绝不触碰 `--copy-to` 目标目录。新批次先用 `--dry-run` 检查 HIF 复制和 Photos 导入计划，确认后再去掉 `--dry-run`；只有在明确想复制 HIF、但暂不导入 Apple Photos 时，才用 `--photos-dry-run`。Photos 导入可能触发 macOS 自动化权限提示，且去重不保证完全可靠。如果最终 `.xmp` 暴露出新的用户风格规律，应直接更新仓库 profile、preset notes、prompt 和记忆。不要把风格学习作为第三个独立流程，也不要再要求用户单独“提取 featured”；以后称为“成片归档”。Lightroom 生成的 `*-Pano.dng` 没有对应 HIF 是正常的，不需要报告为问题。
```
